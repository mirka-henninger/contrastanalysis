...
cont_result <- contrast_dependent(nGroup, lambda,dat)
...
expect_equal(cont_result$contrast.estimate, anova_result$estimate, tolerance = 1e-4)
# equal p values (here tolerance is higher, because p-values may differ due to differences in degrees of freedom)
expect_equal(cont_result$p, anova_result$p.value, tolerance = 1e-1)
# squared t value is F value
expect_equal(cont_result$F, cont_result$t^2, tolerance = 1e-4)
...
compare_result <- compare_dependent(nGroup, lambda1, lambda2, dat)

### Tests
# squared t value is F value
expect_equal(compare_result$results$F,
             compare_result$results$t^2, tolerance = 1e-3)
# difference in contrast wteights
expect_equal(compare_result$contrastWeights$lambda1Std - compare_result$contrastWeights$lambda2Std,
             compare_result$contrastWeights$lambdaDiff, tolerance = 1e-4)
...
### compare_dependent
# group number is not correct
expect_error(compare_dependent(nGroup-1, lambda1, lambda2, dat))
expect_error(compare_dependent(nGroup+1, lambda1, lambda2, dat))

# lambda does not match group
expect_error(compare_dependent(nGroup,  lambda1[-1], lambda2, dat))
expect_error(compare_dependent(nGroup,  lambda1, lambda2[-1], dat))

# dat has strange format
expect_error(compare_dependent(nGroup, lambda1, lambda2, cbind(dat, dat[,1])))
expect_error(compare_dependent(nGroup, lambda1, lambda2, dat[,-1]))
expect_error(compare_dependent(nGroup, lambda1, lambda2, dat[,-4]))
